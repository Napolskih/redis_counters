  ...
  context 'when auto expire enabled' do
    context 'when expire given in counter options' do
      ...
      before do
        ...
        Timecop.freeze(5.seconds.since)

        expect(counter.has_value?(:value => 1)).to be_true
        expect(counter.has_value?(:value => 2)).to be_true
        expect(counter.has_value?(:value => 3)).to be_true
        expect(counter.has_value?(:value => 4)).to be_true

        expect(counter.partitions).to include('part' => 'part1')
        expect(counter.partitions).to include('part' => 'part2')

        expect(counter.data).to include('value' => '1')
        expect(counter.data).to include('value' => '2')
        expect(counter.data).to include('value' => '3')
        expect(counter.data).to include('value' => '4')
